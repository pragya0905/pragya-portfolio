...
**LifeOS** (React 19, Vite, Tailwind CSS, TypeScript AWS SAM backend, Lambda, API Gateway HTTP API, DynamoDB, Cognito, Claude API) — a single-user "operating system for daily life," consolidating task management, journaling, habit tracking, and health logging into one tool backed by real AWS infrastructure, not a toy demo. Authentication is AWS Cognito with custom-built signup/login forms via Amplify's Auth module — no AWS-hosted redirect pages, so it feels like a real product. Also built as a PWA: installs to a phone home screen, opens full-screen, with offline caching and groundwork for push notifications already in place. AI is used in two narrowly-scoped, trust-preserving ways: reading journal entries to auto-detect and tick habit mentions (water intake, exercise) — with manual ticks always winning and never silently overwritten — and suggesting task priority, where a deterministic guardrail computed in code runs first (e.g., a task needing 12 hours due in 20 hours is forced to "High" priority via simple math, not left to a probabilistic model). Per-user data isolation is enforced via the user ID embedded in the Cognito-issued JWT, never trusted from the request itself, verified via API Gateway's native JWT authorizer (not a custom Lambda authorizer) — cross-user isolation is explicitly tested at every step of development. Nothing gets committed until it's deployed to real AWS and verified with real API calls, real Cognito users, and a real browser session. Phase 1 (complete) covers the core loop: task management with manual/AI-suggested priority, typed and voice journaling, manual and AI-detected habit tracking, and a unified daily dashboard. Phase 2 (in progress) is unifying what could have been eight separate bespoke trackers (food, sleep, weight, body fat, mood, call log, expenses, menstrual cycle, medication adherence) under one flexible schema instead — a deliberate architectural choice to make future AI weekly summaries and cross-category pattern detection easier to build later, since they'd only need to query one table.
