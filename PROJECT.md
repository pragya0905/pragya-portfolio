...
**Live site:** https://www.pragyakumari.com
...
12. **A GSAP cinematic redesign, built, shipped, and then fully reverted.** An ambitious scroll-driven "chapters" rewrite of Hero and Experience (pinned sections, `SplitText` word-stagger, scroll-exit timelines) was designed, built, and deployed — then a real production bug surfaced (Hero content stuck invisible after scrolling down and back up, a stale `ScrollTrigger` pin not reversing cleanly). A first fix attempt (`ScrollTrigger.refresh()` on window load) was tried and verified *not* to fix it via the same repro script that found the bug. Rather than continue live-debugging a shipped regression, the decision was made to revert — first non-destructively (`git revert`, preserving an unrelated wanted commit in between), then later, on request, a full `git reset --hard` back to the last known-good pre-GSAP commit, explicitly discarding even the wanted commit once that tradeoff was confirmed. The lesson: an ambitious visual rewrite is only worth keeping if it's actually more robust than what it replaced, not just more impressive in a demo.
13. **A full content refresh from an updated resume** — role changed from an engineering title to Founder, three new projects added, skills and certifications updated, profile photo replaced (later replaced again with a transparent/background-removed version, with the photo frame's background color made theme-aware — dark navy in dark mode, blending into the page canvas in light mode, rather than a fixed dark box that looked wrong against a light background).
14. **Three measured performance fixes** — the particle background's ~8 sequentially-loaded dynamic-import chunks collapsed into one via a Vite `manualChunks` config (~700ms → ~330ms); the profile photo sped up via `<link rel="preload">` + re-encoding (~2410ms → ~222ms); a `<picture>` + WebP/PNG fallback added after a user-flagged browser-compatibility gap. Each fix was measured before and after via Playwright network-waterfall/CDP throttling, not assumed from the change alone.
15. **Custom domain migration** — `pragyakumari.com` purchased and pointed at the existing CloudFront distribution: an ACM certificate requested and DNS-validated, CloudFront updated with both domain aliases, and GoDaddy DNS/forwarding configured (apex 301-redirects to `www`, since GoDaddy's classic DNS has no ALIAS/ANAME support at the zone root). SEO metadata (canonical URL, OG/Twitter tags, JSON-LD) updated to the new domain, along with stale "Founding Engineer" wording left over from the role change in step 13.
16. **A full site audit surfaced two real bugs, both fixed same-day**: the chatbot/contact-form API's CORS `AllowOrigins` allowlist hadn't been updated for the new domain, silently breaking the chatbot's LLM-backed responses (FAQ-only replies kept working, masking the break) — fixed by adding both domain variants to the allowlist, verified with a real cross-origin `fetch()` from the live domain. Separately, the CloudWatch dashboard's contact-form widget was found querying the wrong AWS region (`us-east-1`, copy-pasted from the CloudFront widgets next to it) instead of the region the Lambda actually publishes to (`ap-southeast-2`) — the widget had been silently empty despite real submissions existing. Both bugs had been live for a while without being noticed; found only by deliberately testing the real, deployed system end to end rather than trusting that a working build meant a working product.
17. **Chatbot hardening**: the backend knowledge base (`knowledge.js`) was manually re-synced after step 13's content changes exposed how easily it drifts from `content.js` (see §10). An explicit guardrail was added to the system prompt after live-testing showed the model would already refuse to fabricate negative claims about Pragya when asked — the behavior worked, but only because Claude inferred it from adjacent rules, not because it was a written rule; made explicit and re-verified against a differently-phrased adversarial prompt afterward.
...
**A third bug, found much later during a full-site audit:** the dashboard's "Contact Form Submissions" widget was querying the metric in `us-east-1` — the same region as the CloudFront widgets next to it — but the Lambda that publishes the metric runs (and publishes) in `ap-southeast-2`. CloudFront metrics genuinely always live in `us-east-1` regardless of the distribution's actual region, which is why that region setting was correct for the widgets around it; it just didn't generalize to a Lambda-region metric placed in the same dashboard. The widget had been silently empty for as long as it existed, despite real contact-form submissions succeeding the whole time — fixed by pointing that one widget at the correct region. The dashboard was also expanded with a second section covering the chatbot backend (Lambda invocations/errors/duration, API Gateway count/4xx/5xx/latency), which had no observability at all before this.
...
- **CORS locked to an explicit allowlist.** The API Gateway's CORS config allows only the site's real origins — the CloudFront URL, `https://www.pragyakumari.com`, `https://pragyakumari.com`, and `localhost` for dev — not a wildcard. A request from any other origin gets a response with no `Access-Control-Allow-Origin` header, which browsers refuse to expose to JS, even though the raw HTTP request still returns 200 (this was explicitly verified, not assumed). This allowlist has to be updated by hand whenever a new origin needs access — it was missed during the custom-domain migration and silently broke the chatbot for a period (see §4, step 16) until an audit caught it.
...
- **API Gateway's 4xx/5xx metrics can't detect a CORS misconfiguration.** A rejected cross-origin request still gets a `200` from API Gateway — the browser blocks the *response* from reaching JS, but the server-side metrics look completely healthy. The CORS-allowlist bug in §4 (step 16) would have been invisible to the CloudWatch dashboard even after the dashboard's other bugs were fixed; catching that class of failure would require an external synthetic check (a scheduled call with the production `Origin` header, alerting if the CORS header is missing), which hasn't been built since it hasn't been needed yet.
...
The site now runs on its own custom domain (`pragyakumari.com` / `www.pragyakumari.com`) rather than the raw CloudFront URL, with a proper ACM certificate and GoDaddy DNS pointed at the existing infrastructure — no new hosting stack, just a real domain in front of what was already built. A full site audit (content accuracy, live console/network checks, CORS, and CloudWatch dashboard correctness) was run after the domain migration specifically because a URL change is exactly the kind of thing likely to break a hardcoded allowlist somewhere — which it did (§4, step 16) — and the audit caught it same-day rather than leaving it to be discovered by a visitor.
